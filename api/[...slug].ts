...
import { isOpenRouterConfigured, getOpenRouterStatus, generateOpenRouterResponse } from "../ai-provider";
const rawOpenRouterKey = (process.env.OPENROUTER_API_KEY || process.env.VITE_OPENROUTER_API_KEY || "").trim();
const aiClient: any = isOpenRouterConfigured() ? {} : null;

function sanitizeOpenRouterModel(model: any): string {
  const rawModel = typeof model === "string" ? model.trim() : String(model || "").trim();
  const cleaned = rawModel.replace(/^models\//i, "");
  const forbidden = /(google|vertex|genai)/i;
  if (!cleaned || (forbidden.test(cleaned) && !/^openrouter\//i.test(cleaned))) {
    if (cleaned) {
      console.warn(`[OpenRouter Model Sanitization] Unsafe provider model detected in runtime environment: "${cleaned}". Overriding to openrouter/free.`);
    }
...
const OPENROUTER_PRIMARY_MODEL = sanitizeOpenRouterModel(process.env.MODEL || process.env.OPENROUTER_MODEL || "openrouter/free");
const OPENROUTER_RETRY_COUNT = Number(process.env.OPENROUTER_RETRY_COUNT || 3) || 3;
...
async function openRouterGenerate(payload: any): Promise<any> {
  const model = sanitizeOpenRouterModel(payload?.model || OPENROUTER_PRIMARY_MODEL || "openrouter/free");
  return generateOpenRouterResponse({ ...payload, model });
}
...
const STORAGE_BUCKET = process.env.SUPABASE_STORAGE_BUCKET || process.env.VITE_SUPABASE_STORAGE_BUCKET || "verification";
...
async function ensureStorageBucketExists(bucketName: string) {
  if (!supabaseClient) {
    throw new Error("Supabase service is not configured. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.");
  }

  const { data: buckets, error: listError } = await supabaseClient.storage.listBuckets();
  if (listError) {
    throw new Error(`Unable to list Supabase buckets: ${listError.message || String(listError)}`);
  }

  const exists = Array.isArray(buckets) && buckets.some((bucket: any) => bucket.name === bucketName);
  if (!exists) {
    throw new Error(`Supabase storage bucket '${bucketName}' not found. Create the bucket in Supabase Storage or set SUPABASE_STORAGE_BUCKET/VITE_SUPABASE_STORAGE_BUCKET to the correct name.`);
  }
...
// OpenRouter integration via shared ai-provider
const NODE_ENV = process.env.NODE_ENV || "development";

// Log startup diagnostics for OpenRouter
if (typeof window === "undefined") {
  const status = getOpenRouterStatus();
  if (status.active) {
    console.log("✅ OpenRouter API Loaded: YES");
    console.log(`📊 Current OpenRouter Model: ${status.model}`);
    console.log("🔐 Current API Key Status: ACTIVE");
    console.log(`🌍 Current Environment: ${NODE_ENV === "production" ? "Production" : "Development"}`);
  } else {
    console.log("❌ OpenRouter API Loaded: NO");
...
async function performValidationAndSelfHealing(generated: any, prompt: string, attempt = 1): Promise<{
  ...
  try {
    ...
    const response = await openRouterGenerate({
...
async function handleVerificationEnsureBucket(req: any, res: any) {
  try {
    if (!supabaseClient) {
      return sendJson(res, { error: 'Supabase service client is not configured.' }, 503);
    }
    await ensureStorageBucketExists(STORAGE_BUCKET);
    return sendJson(res, { success: true, bucket: STORAGE_BUCKET });
  } catch (err: any) {
    return sendJson(res, { error: err.message || 'Failed to ensure storage bucket' }, 500);
  }
}
...
function handleApiKeyStatus(res: any) {
  const status = getOpenRouterStatus();
  return sendJson(res, { active: status.active, model: status.model, provider: status.provider });
}

async function handleOpenRouterDiagnostics(res: any) {
  const status = getOpenRouterStatus();
  return sendJson(res, {
    openRouterLoaded: status.active,
    model: status.model,
    provider: status.provider,
    apiKeyStatus: status.active ? "ACTIVE" : "INVALID",
    environment: NODE_ENV,
    apiKeyPrefix: rawOpenRouterKey ? rawOpenRouterKey.substring(0, 10) + "..." : "MISSING",
    timestamp: new Date().toISOString(),
  });
}
...
async function handleAnalyzePrompt(res: any, body: any, prompt: string) {
  ...
  console.log(`[API Handler] OpenRouter Model: ${OPENROUTER_PRIMARY_MODEL}, API Key Loaded: ${rawOpenRouterKey ? "YES" : "NO"}`);
...
async function handleSriAi(req: any, res: any, body: any) {
  console.log(`[Sri AI] Provider: OpenRouter | Model: ${OPENROUTER_PRIMARY_MODEL} | API Key: ${rawOpenRouterKey ? '✅ LOADED' : '❌ MISSING'}`);
  ...
  if (!isOpenRouterConfigured()) {
    return sendJson(res, { error: "OpenRouter API key is not configured. Please set OPENROUTER_API_KEY in your environment and restart the server." }, 500);
  }
  ...
  try {
    ...
    return sendJsonError(res, err, 500);
  }
}

async function handleGenerate(req: any, res: any, body: any) {
  console.log("GENERATE ROUTE EXECUTED");
  console.log(`
╔════════════════════════════════════════════════════════════════╗
║              🟢 PROJECT GENERATION INITIATED                  ║
║                (OpenRouter AI Gateway ACTIVE)                 ║
╚════════════════════════════════════════════════════════════════╝
[Provider] OpenRouter AI Gateway (OpenRouter enforced)
[Provider Status] ✅ ACTIVE AND CONFIGURED
[Model] ${OPENROUTER_PRIMARY_MODEL}
[API Key] ${rawOpenRouterKey ? "✅ LOADED" : "❌ MISSING"}
[Endpoint] https://openrouter.ai/api/v1/chat/completions
[External Provider Detection] ❌ NOT DETECTED (OpenRouter only)
[External Provider Usage] ❌ DISABLED
[External Provider Endpoints] ❌ NOT USED
[External Provider Models] ❌ NOT USED
[Timestamp] ${new Date().toISOString()}
════════════════════════════════════════════════════════════════

console.log: Provider Debug = OpenRouter
console.log: Model = ${OPENROUTER_PRIMARY_MODEL}
console.log: API Key Configured = ${rawOpenRouterKey ? 'YES' : 'NO'}
  `);
  ...
  if (!isOpenRouterConfigured()) {
    return sendJson(res, { error: "OpenRouter API key is not configured. Please add your OPENROUTER_API_KEY in the Secrets panel." }, 500);
  }
  ...
  try {
    ...
    if (!resultText) throw new Error("No response returned from OpenRouter API");
    ...
    return sendJsonError(res, error, 500);
  }
}
...
async function handleRefine(req: any, res: any, body: any) {
  ...
  if (!isOpenRouterConfigured()) {
    return sendJson(res, { error: "OpenRouter API key is not configured." }, 500);
  }
  const projects = getProjects();
...
function sendJsonError(res: any, err: any, defaultStatus = 500) {
  const status = err?.statusCode || defaultStatus;
  return sendJson(res, { error: err?.message || "Internal server error" }, status);
}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (endpoint === "verification/ensure-bucket" && method === "GET") return await handleVerificationEnsureBucket(req, res);
    ...
    if (endpoint === "openrouter-diagnostics" && method === "GET") return handleOpenRouterDiagnostics(res);
