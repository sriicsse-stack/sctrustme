...
import { generateOpenRouterResponse, getOpenRouterStatus } from "./ai-provider";
...
// Runtime: enforce OpenRouter provider for all generation requests
const NODE_ENV = process.env.NODE_ENV || "development";
const openRouterStatus = getOpenRouterStatus();
if (openRouterStatus.active) {
  console.log("✅ OPENROUTER AI ACTIVE");
  console.log(`📊 Current OpenRouter Model: ${openRouterStatus.model}`);
  console.log("🔐 Current API Key Status: ACTIVE");
  console.log(`🌍 Current Environment: ${NODE_ENV === "production" ? "Production" : "Development"}`);
} else {
  console.warn("❌ OpenRouter API key missing or placeholder detected. Set OPENROUTER_API_KEY in environment to enable OpenRouter.");
  console.log("✅ OPENROUTER AI ACTIVE: NO");
  console.log(`🔐 Current API Key Status: INVALID`);
}

async function generateContentViaOpenRouter(payload: any): Promise<any> {
  // Accept legacy payload shapes that include `contents` (array) or `prompt`.
  let adaptedPayload = { ...payload };
  try {
    if (Array.isArray(payload?.contents)) {
      // Convert contents array into a single prompt string for OpenRouter
      const parts: string[] = [];
      for (const item of payload.contents) {
        if (typeof item === 'string') parts.push(item);
        else if (item?.parts && Array.isArray(item.parts)) {
          parts.push(item.parts.map((p: any) => p?.text || '').filter(Boolean).join('\n'));
        } else if (item?.text) parts.push(String(item.text));
      }
      adaptedPayload = { ...adaptedPayload, prompt: parts.join('\n\n') };
    }

    console.log("OUTBOUND URL = https://openrouter.ai/api/v1/chat/completions");
    const resp = await generateOpenRouterResponse(adaptedPayload);
    console.log("MODEL =", resp?.model || "openrouter/free");
    console.log("PROVIDER =", "OpenRouter");
    return resp;
  } catch (err: any) {
    console.error("[OpenRouter Error]", String(err?.message || err || ""));
    throw err;
  }
}
...
const VERIFICATION_FILE = path.join(process.cwd(), "verification_requests.json");
...
function getVerificationRequests() {
  if (!fs.existsSync(VERIFICATION_FILE)) fs.writeFileSync(VERIFICATION_FILE, JSON.stringify([]));
  try { return JSON.parse(fs.readFileSync(VERIFICATION_FILE, 'utf-8')); } catch(e) { return []; }
}
function saveVerificationRequests(items:any[]) { fs.writeFileSync(VERIFICATION_FILE, JSON.stringify(items, null, 2)); }
...
app.post("/api/razorpay/create-order", async (req, res) => {
  try {
    ...
    if (!plan || typeof plan !== "string") return res.status(400).json({ error: "Plan is required and must be a string." });
...
app.post("/api/razorpay/verify", async (req, res) => {
  try {
    ...
    // Optionally double-check payment status with Razorpay API when client present
    ...
    const paymentStatus = paymentDetails?.status || (req.body.status || "captured");
    const amount = paymentDetails?.amount || req.body.amount || orderDetails?.amount || null;
    ...
    // Activate plan and update credits based on plan only if payment was captured
    ...
    if (paymentStatus === "captured" || paymentStatus === "authorized") {
      ...
      saveUserState(state);
      return res.json({ success: true, txn });
    }

    // If payment not captured, respond accordingly
    saveUserState(state);
    return res.status(400).json({ error: "Payment not captured", status: paymentStatus, txn });
  } catch (err: any) {
...
app.post('/api/feedback/submit', (req, res) => {
  try {
    ...
    let list = [];
    try { list = fs.existsSync(FEEDBACK_FILE) ? JSON.parse(fs.readFileSync(FEEDBACK_FILE,'utf-8')) : []; } catch(e: any) { list = []; }
    list.unshift(fb);
...
app.get('/api/feedback/list', (req, res) => {
  try { const list = fs.existsSync(FEEDBACK_FILE) ? JSON.parse(fs.readFileSync(FEEDBACK_FILE,'utf-8')) : []; res.json(list); } catch(e: any){ res.status(500).json({ error: e.message||'fail' }); }
});
...
app.post('/api/feedback/update', (req, res) => {
  ...
  } catch(e: any){ res.status(500).json({ error: e.message||'fail' }); }
});
...
app.post('/api/referral/apply', (req, res) => {
  try {
    ...
    if (!isMatchingReferredUser) {
      return res.status(400).json({ error: 'Referral must be claimed by the signed-in referred user.' });
    }
...
app.post('/api/verification/submit', (req, res) => {
  try {
    const currentUser = getAuthenticatedRequestUser(req, req.body);
    if (!currentUser) {
      return res.status(401).json({ error: 'Authentication required to submit verification.' });
    }

    const { documentType, documentName, documentPath, documentMimeType, note, documentUrl } = req.body || {};
    if (!documentType || !documentName || (!documentPath && !documentUrl)) {
      return res.status(400).json({ error: 'documentType, documentName and documentPath or documentUrl are required.' });
    }
    ...
    res.status(500).json({ error: e.message || 'Verification submission failed' });
  }
});
...
app.get('/api/verification/me', (req, res) => {
  try {
    ...
    res.json({ success: true, verificationRequest: request, verificationStatus: request?.status || 'none' });
  } catch (e: any) {
...
app.post('/api/verification/review', (req, res) => {
  try {
    const currentUser = getAuthenticatedRequestUser(req, req.body);
    if (!currentUser) {
      return res.status(401).json({ error: 'Authentication required to review verification requests.' });
    }

    const { requestId, status, reviewerNotes } = req.body || {};
    if (!requestId || !['approved', 'rejected'].includes(status)) {
      return res.status(400).json({ error: 'requestId and status (approved/rejected) are required.' });
    }
    ...
    res.status(500).json({ error: e.message || 'Verification review failed' });
  }
});
...
app.get("/api/api-key-status", (req, res) => {
  const openStatusNow = getOpenRouterStatus();
  const hasKey = !!openStatusNow.active;
  const razorpayConfigured = !!(RAZORPAY_KEY_ID && RAZORPAY_KEY_SECRET && razorpayClient);
  res.json({ active: hasKey, razorpay: { configured: razorpayConfigured, keyIdPresent: !!RAZORPAY_KEY_ID } });
});
        ...
        responseSchema: {
          ...
          properties: {
            ...
            cost: {
              ...
              properties: {
                apiCallCost: { type: "string" },
                hostingCost: { type: "string" },
                databaseCost: { type: "string" }
              }
            },
...
app.post("/api/sri-ai", async (req, res) => {
  ...
  if (!openRouterStatus.active) {
    return res.status(500).json({
      error: "OpenRouter API key is not configured. Please set OPENROUTER_API_KEY in your environment and restart the server.",
    });
  }
...
${logDump || "None provided"}`;
    ...
    // Map conversation history to OpenRouter multi-turn format: Content[]
    const contents: any[] = [];
    ...
    const result = await generateContentViaOpenRouter({
      contents: contents,
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            reply: { type: "string", description: "Your friendly multilingual markdown explanation or diagnostic answer. Address user naturally as human." },
            fixAvailable: { type: "boolean", description: "Whether an automated edit can resolve this problem" },
            suggestedFixPrompt: { type: "string", description: "A highly specific 1-sentence prompt to execute if the user presses 'Auto-Fix'" }
          }
        }
      }
    });
...
async function performValidationAndSelfHealing(generated: any, prompt: string, attempt = 1): Promise<{
  ...
  if (errors.length === 0 || !openRouterStatus.active || attempt > 2) {
    if (errors.length > 0) {
      report.status = "fail";
    ...
    const response = await generateContentViaOpenRouter({
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            name: { type: "string", description: "Clear, clean, literal name of the generated website (e.g. 'SaaS Dashboard', 'Hotel Reservation System')" },
            description: { type: "string", description: "A one-sentence description of the website's purpose" },
            analysis: {
              type: "object",
              required: ["features", "database", "apis", "security"],
              properties: {
                features: { type: "array", items: { type: "string" }, description: "List of custom interactive features designed" },
                database: { type: "array", items: { type: "string" }, description: "Database collections or tables required (e.g. 'users', 'posts', 'bookings')" },
                apis: { type: "array", items: { type: "string" }, description: "API routes designed (e.g. 'GET /api/v1/videos')" },
                security: { type: "string", description: "Security controls built-in or structured" },
              }
            },
            files: {
              type: "array",
              ...
              items: {
                ...
                required: ["path", "content", "language"],
                properties: {
                  path: { type: "string", description: "Relative file path (e.g. 'src/app/page.tsx', 'supabase/schema.sql', 'server/index.js')" },
                  content: { type: "string", description: "Complete, production-ready source code with zero omissions" },
                  language: { type: "string", description: "Language tag (typescript, javascript, sql, json, css, markdown)" }
                }
              }
            },
            previewHtml: {
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            description: { type: "string", description: "A sentence describing the project" },
            files: {
              type: "object",
              properties: {
                name: { type: "string", description: "Name of the website" },
